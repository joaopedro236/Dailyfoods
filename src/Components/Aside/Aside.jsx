...
export default function Aside({ setState, state, user, onHome }) {
    ...
    useEffect(() => {
        ...
        document.body.style.setProperty(
            'padding-left',
            user === true && media.matches ? '80px' : '0px'
        )

    }, [user])
    return (
        <>
            <section className={`aside w-52 hidden duration-300 fixed top-0 left-0 h-screen ${user ? 'flex' : '!hidden'} flex-col z-40 p-5 justify-between overflow-y-auto overflow-x-hidden ${asideActive ? '!w-20' : 'w-52'} z-20`}>
