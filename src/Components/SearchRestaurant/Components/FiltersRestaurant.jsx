...
export default function Filters_Restaurant({stateSection,state,setState,filterOrdersExists ,setFilterOrdersExists}){
    return (
        <>
        <section className={`filtersRestaurant duration-300 ${state == true && stateSection == 2? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} flex gap-12  flex-col pt-[17px] z-[90] absolute top-0 left-0 w-full h-screen`}>
            ...
            <div className="filtersContent flex flex-col gap-2 px-2 w-full">
                <div className="filter flex items-center justify-between px-2 w-full">
                    <button className='flex items-center py-3 justify-between w-full px-3' onClick={() => setFilterOrdersExists(prev => !prev)}> 
